...
namespace LijanTeck.Controllers
{
    ...
    public class PlansAdminController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] PlanUpsertDto dto, CancellationToken ct)
            => Ok(new { PlanId = await _svc.CreateAsync(dto, ct) });

        [HttpPut("{id:long}")]
        public async Task<IActionResult> Update(long id, [FromBody] PlanUpsertDto dto, CancellationToken ct)
